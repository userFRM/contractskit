...
//!
//! ```text
//! $CONTRACTSKIT_CACHE_DIR/          (default: XDG cache / contractskit)
//! ├── contracts-2024.parquet        ← cached body
//! ├── contracts-2024.parquet.etag
//! ├── contracts-2023.parquet
//! └── contracts-2023.parquet.etag
//! ```
//!
//! # Fetch flow (per call)
//!
...
impl CachedFetcher {
    pub fn new(http: reqwest::Client, base_url: String, cache_dir: PathBuf) -> Self {
        let mirror_url = Some(
            std::env::var("CONTRACTSKIT_MIRROR_URL")
    ...
    /// Fetch a parquet file by logical key (e.g. `"contracts-2024"`).
    ///
...
///
/// Priority:
/// 1. `$CONTRACTSKIT_CACHE_DIR` env var.
/// 2. XDG/platform cache dir for the `contractskit` application.
/// 3. Fallback: `~/.cache/contractskit`.
pub(crate) fn default_cache_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("CONTRACTSKIT_CACHE_DIR") {
        return PathBuf::from(dir);
    }
    if let Some(proj) = directories::ProjectDirs::from("", "", "contractskit") {
...
fn dirs_fallback() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        std::env::var("LOCALAPPDATA")
            .map(|d| PathBuf::from(d).join("contractskit").join("cache"))
            .unwrap_or_else(|_| PathBuf::from("contractskit-cache"))
    }
    #[cfg(not(target_os = "windows"))]
    {
        std::env::var("HOME")
            .map(|h| PathBuf::from(h).join(".cache").join("contractskit"))
            .unwrap_or_else(|_| PathBuf::from(".contractskit-cache"))
    }
}
...
mod tests {
    ...
    async fn etag_304_returns_cached() {
        ...
        let body = b"contracts-parquet-bytes";
        ...
        let first = fetcher.fetch("contracts-2024").await.unwrap();
        assert_eq!(first.as_ref(), body);
        let second = fetcher.fetch("contracts-2024").await.unwrap();
    ...
    async fn stale_cache_fallback() {
        ...
        let server = MockServer::start().await;
        let stale_body = b"stale-contracts-data";
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        let cache_file = cache_dir.path().join("contracts-2024.parquet");
        tokio::fs::write(&cache_file, stale_body).await.unwrap();

        let mut fetcher = CachedFetcher::new(http, server.uri(), cache_dir.path().to_path_buf());
        fetcher.set_mirror_url(None);

        let result = fetcher.fetch("contracts-2024").await.unwrap();
    ...
    async fn corrupt_stale_cache_rejected_when_manifest_present() {
        ...
        let manifest_json = format!(r#"{{"contracts-2024.parquet":"sha256:{correct_digest}"}}"#);
        ...
        let http = reqwest::Client::builder()
            ...
            .unwrap();
        let cache_dir = tempfile::TempDir::new().unwrap();
        tokio::fs::write(cache_dir.path().join("contracts-2024.parquet"), b"CORRUPT")
            .await
            .unwrap();
