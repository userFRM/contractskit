//! Unified error type for contractskit. All public methods return `contractskit::Result<T>`.
use thiserror::Error;
...
mod tests {
    ...
    fn checksum_mismatch_displays_both_digests() {
        let e = Error::ChecksumMismatch {
            file: "contracts-2024.parquet".into(),
            expected: "aaa".into(),
            actual: "bbb".into(),
        };
        let s = e.to_string();
        assert!(s.contains("aaa") && s.contains("bbb") && s.contains("contracts-2024.parquet"));
    }
}
